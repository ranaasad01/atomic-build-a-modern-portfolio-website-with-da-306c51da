...
export default function Navbar() {
  ...
  const getLabel = (link: { label: string; href: string; type: string }) => {
    const key = `nav.${link.label.toLowerCase()}`;
    try {
      return t(key);
    } catch {
      return link.label;
    ...
    >
      <nav className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3 flex-shrink-0">
          <Link
            href="/contact"
            className="px-4 py-2 text-sm font-semibold rounded-lg bg-[#e94560] text-white hover:bg-[#e94560]/90 transition-all duration-200 shadow-[0_0_20px_rgba(233,69,96,0.3)] hover:shadow-[0_0_28px_rgba(233,69,96,0.5)]"
          >
            {t("nav.hire")}
          </Link>
        </div>
            ...
            <ul className="max-w-7xl mx-auto px-6 py-4 flex flex-col gap-1">
                ...
                <li key={link.href}>
                  <Link
                    ...
                    className={`block px-4 py-3 text-sm font-medium rounded-lg transition-all duration-200 ${
                      isActive(link.href)
                        ? "text-[#e94560] bg-[#e94560]/10 border border-[#e94560]/20"
                        : "text-white/60 hover:text-white hover:bg-white/5"
                    }`}
                  >
                    {getLabel(link)}
              ...
              <li className="pt-2">
                <Link
                  ...
                  className="block px-4 py-3 text-sm font-semibold rounded-lg bg-[#e94560] text-white text-center hover:bg-[#e94560]/90 transition-all duration-200"
                >
                  {t("nav.hire")}
                </Link>
              </li>
